...
def verifier_circuit_suppression(matrice):
    ...
    print("\nDétection de circuit par suppression des points d'entrée :")
    while restants:
        entrees = [i for i in restants if indegres[i] == 0]
        if not entrees:
            print("-> Circuit détecté")
            return False

        print("Points d'entrée :", " ".join(map(str, entrees)))
...
if __name__ == "__main__":
    ...
    #PARTIE 1
    fichier = "Tables de contraintes/table 3.txt" 
    contraintes = lire_contraintes(fichier)
    print("Contraintes stockées en mémoire :")
    for k, v in contraintes.items():
        print(f"Tache {k}: Durée = {v['duree']}, Prédécesseurs = {v['pred']}")
